...
i32 main() {
    ...
    ld cntRub = 100, cntDollar = 0, cntEuro = 0;
    ...
    for (int i = 0; i < n - 1; ++i) {
        ld dollarProfit = c[i+1].first / c[i].first;
        ld euroProfit = c[i+1].second / c[i].second;
        if (dollarProfit <= 1 && euroProfit <= 1) {
            cntRub += cntEuro * c[i].second + cntDollar * c[i].first;
            cntEuro = 0;
            cntDollar = 0;
        }
        else if (dollarProfit > euroProfit) {
            ld curRub = cntRub + cntEuro * c[i].second;
            cntDollar += curRub / c[i].first;
            cntRub = 0;
            cntEuro = 0;
        }
        else {
            ld curRub = cntRub + cntDollar * c[i].first;
            cntEuro += curRub / c[i].second;
            cntRub = 0;
            cntDollar = 0;
        }
    }

    cout << fixed << setprecision(2) << cntRub + cntDollar * c.back().first + cntEuro * c.back().second << en;
