...
i32 main() {
  ...
  //  freopen("output.txt", "wt", stdout);
    ...
    for (int i = 0; i < n; ++i) {
        sums[i] = gansters[i][1];
        for (int j = 0; j < i; ++j) {
            if (abs(gansters[i][2] - gansters[j][2]) <= abs(gansters[i][0] - gansters[j][0])) {
                if (sums[j] + gansters[i][1] >  sums[i]) {
                    sums[i] = sums[j] + gansters[i][1];
                }
            }
        }
    }
