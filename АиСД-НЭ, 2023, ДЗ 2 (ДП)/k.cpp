...
i32 main() {
  ...
  //  freopen("output.txt", "wt", stdout);
    ...
    vvi clients(1+n, vi(3));
    for (int i = 1; i <= n; ++i) {
        cin >> clients[i][0] >> clients[i][1] >> clients[i][2];
    }

    for (int i = 1; i <= n; ++i) {
        times[i] = times[i-1] + clients[i][0];
        if (i - 2 >= 0 && times[i-2] + clients[i-1][1] < times[i]) {
            times[i] =  times[i-2] + clients[i-1][1];
        }
        if (i - 3 >= 0 && times[i-3] + clients[i-2][2] < times[i]) {
            times[i] =  times[i-3] + clients[i-2][2];
        }
    }
