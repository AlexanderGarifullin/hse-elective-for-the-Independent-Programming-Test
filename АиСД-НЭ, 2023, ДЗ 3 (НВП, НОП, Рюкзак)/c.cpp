...
i32 main() {
    ...
    while(t--){
        ...
        vi v(n); fin(v);
        // first - cnt fullused
        // second weight in last
        vpii d ( 1 << n, {1e9 + 1, 1e9 + 1});
        d[0] = {1,0};
        ...
        cout << d.back().first << en;
        int u = (1 << n) - 1;
        vi path = {(1 << n) - 1};
        while (u != 0) {
            if (d[pred[u]].first < d[u].first) path.push_back(pred[u]);
            u = pred[u];
        }
        path.push_back(0);
        reverse(all(path));
        vvi ans(isz(path)- 1);
        for (int i = 0; i < isz(path) - 1; ++i) {
            int dif = path[i+1] - path[i];
            for (int j = 0; j <= __lg(dif) ; ++j) {
                if ((dif & (1 << j)) > 0) {
                    ans[i].push_back(j + 1);
                }
            }
