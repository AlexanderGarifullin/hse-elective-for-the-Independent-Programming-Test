...
i32 main() {
    ...
    for (int i = 0; i < e2 - e1 ; ++i) {
        ...
        for (int j = 1; j <= n; ++j) {
            int step =wi[j];
            if (i + step > e2 - e1) continue;
            setmax(dp[i + step], dp[i] + pi[j]);
        }
    }
    if (dp.back() == -1) {
        cout << "This is impossible." << en;
        return 0 ;
    }
    vi dp2 (e2 - e1 + 1, INT_MAX);
    dp2[0] = 0;
    for (int i = 0; i < e2 - e1 ; ++i) {
        if (dp2[i] == INT_MAX) continue;
        for (int j = 1; j <= n; ++j) {
            int step =wi[j];
            if (i + step > e2 - e1) continue;
            setmin(dp2[i + step], dp2[i] + pi[j]);
        }
    }
    cout << dp2.back() << ' ' << dp.back() << en;
