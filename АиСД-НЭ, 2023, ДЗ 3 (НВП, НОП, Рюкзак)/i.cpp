...
i32 main() {
    ...
    vvi v(isz(s2) + 1, vi(isz(s1) + 1));
    V<V<pii>> p(isz(s2) + 1, V<pii>(isz(s1) + 1));

    vvi type(isz(s2) + 1, vi(isz(s1) + 1));

    for (int i = 1; i <= isz(s2); ++i) {
        for (int j = 1; j <= isz(s1); ++j) {
            char chars2 = s2[i-1];
            char chars1 = s1[j-1];
            if (chars1 == chars2) {
                v[i][j] = v[i-1][j-1] + 1;
                ...
                if (v[i-1][j] == 0 && v[i][j-1] == 0) continue;
                if (v[i-1][j] > v[i][j-1]) {
                    v[i][j] = v[i-1][j];
                    p[i][j] = pii(i-1, j);
                }
...
//    for (int i = 0; i < isz(s2) + 1; ++i) {
//        for (int j = 0; j < isz(s1) + 1; ++j) {
//            cout << v[i][j] << ' ';
//        }
//        nline;
//    }
...
//    cout << jj << en;
    ...
    while (ii != 0 && jj != 0) {
        ...
        int newi = p[ii][jj].first;
        int newj = p[ii][jj].second;
        ii = newi;
        jj = newj;
    }
